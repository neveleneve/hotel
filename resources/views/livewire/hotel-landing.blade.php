            ...
            <a class="max-w-xs h-full flex flex-col bg-white border border-gray-200 rounded-lg shadow lg:mb-3"
                ...
                <div class="relative w-full">
                    ...
                    <img class="w-full aspect-square object-cover rounded-t-lg"
                        src="{{ asset('assets/img/hotel/' . $item->image) }}" alt="Gambar Hotel {{ $item->name }}">
                </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center mt-1">
                            @for ($i = 1; $i <= 5; $i++)
                                ...
                                @elseif ($i - 0.99 <= $item->rating)
                                    <i class="material-icons text-yellow-500 text-sm align-middle">star_half</i>
                                @else
                                    <i class="material-icons text-yellow-500 text-sm align-middle">star_border</i>
                                @endif
                            @endfor
                            <span
                                class="ml-1 text-sm font-semibold text-gray-700">{{ number_format($item->rating, 1, '.', ',') }}</span>
                        </div>
                    </div>
