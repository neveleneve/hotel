...
@push('tab')
    <nav class="fixed bottom-0 left-0 w-full bg-[--on-primary] text-white justify-around py-4 rounded-top z-20">
        <div class="px-4">
            <form method="POST" action="{{ route('hot-sale.order', [$hotSale->user_id, $hotSale->id]) }}"
                class="grid grid-cols-4 gap-4" onsubmit="getData()">
                @csrf
                <input type="hidden" name="hot_sale_id" value="{{ $hotSale->id }}">
